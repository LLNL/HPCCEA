...
def gender(dest, mydb):
    ...
    for attr in gen.getattr_all():
        ...
        if len(result) == 0:
            descrip = None
            add_gender = (
                "INSERT IGNORE INTO GENDER (gender_name, descrip) VALUES (%s, %s)"
            )
...
def deletenodes(dest, present, absent, mydb):
    ...
    for (node_name,) in cursor:
        ...
        elif (node_name not in present) and (node_name not in absent):
...
def deleteconfig(dest, mydb):
    ...
    for node in gen.getnodes():
        ...
        for (gender_name, val) in results:
            if (val is None) and gen.testattr(gender_name, node) == 0:
                query = "DELETE FROM CONFIGURATION WHERE config_id=%s"
                cursor.execute(query, (node + gender_name,))
                mydb.commit()
            elif not (val is None) and gen.testattrval(gender_name, val, node) == 0:
